...
/**
 * Botão "Enviar Notificacao" para ResponsavelRNC (01. Registro de Não Conformidade)
 */
@ActionButton(
    description = "Enviar Notificacao",
    instanceName = "ResponsavelRNC",
    accessControlled = false,
    transactionType = TransactionType.AUTOMATIC,
    refreshType = RefreshTypeEnum.PARENT_ITEM)
public class EnviarNotificacaoResponsavelRNC implements AcaoRotinaJava {
    ...
    public void doAction(ContextoAcao ctx) throws Exception {
        ...
        for (i = (arrayOfRegistro = ctx.getLinhas()).length, b = 0; b < i; ) {
            ...
            String codUsu = linha.getCampo("CODUSU").toString();
            String enviarEmail = linha.getCampo("ENVIAREMAIL").toString();
            try {
                retorno = NotificacaoAcoes.enviaNotificacaoResponsavelRNC(rncId, codUsu, enviarEmail);
                if (retorno != null) {
