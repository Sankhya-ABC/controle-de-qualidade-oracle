...
public class SolicitaEtiquetas {
    ...
    public void setParametros(BigDecimal nunota, Integer qtdEtiquetas) throws Exception {
        ...
        sql.appendSql("\t\t            (SELECT SERV.DESCRICAO FROM SIGSER SERV, SIGCFG CFG WHERE SERV.NUMSERV = CFG.NUMSERV AND ROWNUM = 1)) AS DESCSERVICO, ");
        sql.appendSql("\t\t        NVL((SELECT SERV.IDSERVICO FROM SIGSER SERV WHERE SERV.NUMSERV = NOTA.AD_NUMSERV), ");
        sql.appendSql("\t\t            (SELECT SERV.IDSERVICO FROM SIGSER SERV, SIGCFG CFG WHERE SERV.NUMSERV = CFG.NUMSERV AND ROWNUM = 1)) AS IDSERVICO ");
