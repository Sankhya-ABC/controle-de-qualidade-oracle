...
public class SolicitaEtiquetas {
    ...
    public void setParametros(BigDecimal nunota, Integer qtdEtiquetas) throws Exception {
        ...
        sql.appendSql("            ISNULL(DESTTRAY.CEP,DESTINATARIO.CEP) CEP_DESTINATARIO, ");
        sql.appendSql("\t\t        REMETENTE.CGC AS CNPJ_CPF, ");
        sql.appendSql("\t\t        ISNULL(NOTA.AD_NUMSERV, (SELECT TOP 1 AD_NUMSERV FROM SIGCFG)) AS NUMSERVICO, ");
        sql.appendSql("\t\t        (SELECT TOP 1 NUEMB FROM SIGCFG) AS NUEMB, ");
        sql.appendSql("\t\t        ISNULL((SELECT SERV.DESCRICAO FROM SIGSER SERV WHERE SERV.NUMSERV = NOTA.AD_NUMSERV), ");
        sql.appendSql("\t\t            (SELECT TOP 1 SERV.DESCRICAO FROM SIGSER SERV, SIGCFG CFG WHERE SERV.NUMSERV = CFG.NUMSERV)) AS DESCSERVICO, ");
        sql.appendSql("\t\t        ISNULL((SELECT SERV.IDSERVICO FROM SIGSER SERV WHERE SERV.NUMSERV = NOTA.AD_NUMSERV), ");
        sql.appendSql("\t\t            (SELECT TOP 1 SERV.IDSERVICO FROM SIGSER SERV, SIGCFG CFG WHERE SERV.NUMSERV = CFG.NUMSERV)) AS IDSERVICO ");
